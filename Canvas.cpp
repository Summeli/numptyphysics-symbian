...
#if !defined(WIN32) && !(defined(__APPLE__) && defined(__MACH__)) && !defined(__SYMBIAN32__)
...
#ifdef __SYMBIAN32__
  std::string h( "\\" );  
  std::string f( getResourcePath() + h );
#else
  std::string f( "data/" );
#endif
...
#ifdef __SYMBIAN32__
	 std::string h( "\\" );  
     f = std::string( getSharedResourcePath() + h  );
#else
...
#pragma pack(push,1)
  ...
  //this kind of tricks don't work with symbian compiling system >:)
